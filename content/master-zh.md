...
<!-- TEMPLATE_ONLY
发布源模板。
使用 scripts/validate_claims.py --show-rendered 从
contracts/claim-registry.json 解析所有 {{claim:Cxx:zh}}。
禁止手工修改解析后的性能数字。
-->
...
它也是一个边界刻意收紧的结果。{{claim:C02:zh}} 当前算子契约是 BF16、head dimension 128；测量只覆盖一套 NVIDIA H20 环境中的冻结矩阵。它不代表所有 Hopper GPU、所有 GDN 形状，也不是端到端模型吞吐或延迟结论。
