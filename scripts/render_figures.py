...
EXPECTED_ASSET_SHA256 = {
    ...
    "ratios_by_row.png": "cd5fa77db9c8bc40a7ec1c2abf5b8ba4177db3778cf6b23a8c9243be7c7e9834",
    "ratios_by_row.svg": "aa324e1ab88aa733035c7dbbd9fe2ba42c0b3cdcaed66afc465bcb243cf4f864",
}
PRIVATE_PATTERNS = (
    b"/Users/",
    b"/home/",
...
def validate_outputs(output_dir: Path, *, require_sealed_hashes: bool = False) -> dict[str, Any]:
    errors = []
    hashes = {}
    ...
    if require_sealed_hashes:
        for filename, expected_sha256 in EXPECTED_ASSET_SHA256.items():
            if hashes.get(filename) != expected_sha256:
                errors.append(
                    f"{filename}: checked-in seal differs from the frozen SHA256 {expected_sha256}"
                )
    return {
        ...
        "checked_in_asset_seals_verified": require_sealed_hashes and not errors,
    }
...
def check_determinism(evidence: Evidence, output_dir: Path) -> dict[str, Any]:
    existing = validate_outputs(output_dir, require_sealed_hashes=True)
    ...
    with tempfile.TemporaryDirectory(prefix="gdn-sm90a-figures-check-") as temporary:
        ...
        for name in FIGURE_NAMES:
            ...
            if both_exist and current.read_bytes() != candidate.read_bytes():
                message = f"{filename}: generated bytes differ from the checked-in asset"
                svg_byte_errors.append(message)
                errors.append(message)
    return {
        "status": "PASS" if not errors else "FAIL",
        "errors": errors,
        ...
        "checked_in_asset_seals_verified": existing["checked_in_asset_seals_verified"],
        "svg_deterministic_byte_comparison": not svg_byte_errors,
        "png_cross_platform_policy": "sealed_checked_in_bytes_and_structural_regeneration",
    }
